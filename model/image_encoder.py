...
def build_image_encoder(vit_weights_path=r"/mnt/data/weights/tinyvit.pth"):
    model = TinyViT(img_size=256,
                num_classes=1000,
                ...
                drop_path_rate=0.0)
    weights = torch.load(vit_weights_path, weights_only=True, map_location="cpu")
...
class Image_Encoder(nn.Module):
    def __init__(self, d_model, vit_weights_path=r"/mnt/data/weights/tinyvit.pth"):
        super().__init__()
        self.m = TinyViT(num_classes=1000,
                embed_dims=[64, 128, 160, 320],
                depths=[2, 2, 6, 2],
                num_heads=[2, 4, 5, 10],
                window_sizes=[7, 7, 14, 7],
                drop_path_rate=0.0)
        # self.m = TinyViT(img_size=256, in_chans=3, num_classes=1000,
        #         embed_dims=[64, 128, 160, 320],
        #         depths=[2, 2, 6, 2],
        #         num_heads=[2, 4, 5, 10],
        #         window_sizes=[7, 7, 14, 7],
        ...
        self.embed_dim = 256
        self.conv1 = nn.Conv2d(self.embed_dim, self.embed_dim, 3,2,1, groups=self.embed_dim)
        self.norm1 = nn.SyncBatchNorm(self.embed_dim)
        self.relu1 = nn.ReLU6()
        self.conv2 = nn.Conv2d(self.embed_dim, d_model, 1)
