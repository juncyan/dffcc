...
try:
    ...
    from .mmcv_load_checkpoint import _load_checkpoint
    has_mmseg = True
except ImportError:
...
class DilatedReparamBlock(nn.Module):
    ...
    """
    def __init__(self, channels, kernel_size=9, deploy=False, use_sync_bn=False, attempt_use_lk_impl=True):
        super().__init__()
...
def load_with_key(model, key):
    ...
    if hf_hub_download is not None:
        ...
        checkpoint = torch.hub.load_state_dict_from_url(url=model_urls[key], map_location="cpu", check_hash=True, weights_only=True)
